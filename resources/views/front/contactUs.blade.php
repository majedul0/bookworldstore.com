    ...
    <section class="bg-white py-5 md:py-10">
        <div class="container mx-auto px-4">
            <div class="mb-12 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                <div class="rounded-3xl border-2 border-gray-300 bg-gray-50 p-8 shadow-sm transition-shadow hover:shadow-md">
                    ...
                    @if ($contactPhone)
                        <a href="tel:{{ $contactPhone }}" class="mb-3 flex items-center gap-3">
                            <span class="flex shrink-0 rounded-full bg-black p-2 text-white">
                                <i class="fas fa-phone h-5 w-5 text-center leading-5"></i>
                            </span>
                            <span class="text-sm font-semibold text-black">Phone: {{ $contactPhone }}</span>
                        </a>
                    @endif
                    ...
                    <a href="mailto:{{ $contactEmail }}" class="flex items-center gap-3">
                        ...
                        <span class="break-all text-sm font-semibold text-black">Email: {{ $contactEmail }}</span>
                    </a>
                </div>
                ...
                <div class="rounded-3xl border-2 border-yellow-500 bg-yellow-100 p-8 shadow-sm transition-shadow hover:shadow-md md:col-span-2 lg:col-span-1">
                    ...
                    <div class="flex flex-wrap gap-4">
                        @if ($socials['facebook'] ?? null)
                            <a href="{{ $socials['facebook'] }}" class="rounded-lg bg-black p-4 text-white shadow-md transition-colors hover:bg-gray-800" aria-label="Facebook">
            ...
            <div class="bg-white md:p-5">
                ...
                <form action="{{ route('contactUs.store') }}" method="post" class="mx-auto max-w-4xl space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Name" required class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-black">
                        <input type="tel" name="phone" value="{{ old('phone') }}" placeholder="Phone number" class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-black">
                    </div>

                    <input type="email" name="email" value="{{ old('email') }}" placeholder="E-mail" class="w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-black">

                    <textarea name="message" placeholder="Message" rows="6" required class="w-full resize-none rounded-lg border border-gray-300 px-4 py-3 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-black">{{ old('message') }}</textarea>

                    <button type="submit" class="cursor-pointer rounded-lg bg-black px-8 py-3 font-bold text-white shadow-md transition-colors duration-200 hover:bg-gray-800 hover:shadow-lg">
                        SEND MESSAGE
                    </button>
                </form>
            </div>
